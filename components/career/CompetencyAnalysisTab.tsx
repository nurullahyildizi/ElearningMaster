...
import { Job, SkillCategory, JobAnalysis } from '../../types';
import { JOB_DATA, SKILL_CATEGORIES } from '../../constants';
...
const JobAnalysisComponent: React.FC<{ job: Job }> = ({ job }) => {
    const { currentUser } = useAuth();
    const { skillData } = useData();
    const { openUpgradeModal } = useUI();
    
    const [analysis, setAnalysis] = React.useState<JobAnalysis | null>(null);
    const [loading, setLoading] = React.useState(false);
    const [error, setError] = React.useState<string | null>(null);

    const handleAnalyze = React.useCallback(async () => {
        if (!currentUser || !skillData) return;
        ...
        setLoading(false);
    }, [job, skillData, currentUser, openUpgradeModal]);
        ...
        <div className="mt-6 border-t border-slate-700 pt-6">
            ...
            {!analysis && !loading && (
                ...
                >
                    ...
                    Job-Passung jetzt analysieren {currentUser.subscriptionStatus !== 'pro' && '(Pro-Feature)'}
                </button>
            )}
...
const CompetencyAnalysisTab: React.FC = () => {
    const { skillData } = useData();
